...
with f as (
  ...
  where birth_date between to_date('{{ var("analysis_start") }}')
                      and to_date('{{ var("analysis_end") }}')
),

lk_tempo as (
  select sk_tempo, data_dia from {{ ref('dim_tempo') }}
),

lk_loc as (
  select sk_localidade, municipio_code from {{ ref('dim_localidade') }}
),
